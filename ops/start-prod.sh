...
INDRA_NATS_JWT_SIGNER_PRIVKEY="${INDRA_NATS_JWT_SIGNER_PRIVKEY:-}" # pass this in through CI
INDRA_NATS_JWT_SIGNER_PUBKEY="${INDRA_NATS_JWT_SIGNER_PUBKEY:-}" # pass this in through CI
...
services:
  ...
  node:
    ...
    environment:
      ...
      INDRA_NATS_JWT_SIGNER_PRIVATE_KEY: $INDRA_NATS_JWT_SIGNER_PRIVKEY
      INDRA_NATS_JWT_SIGNER_PUBLIC_KEY: $INDRA_NATS_JWT_SIGNER_PUBKEY
  ...
  nats:
    ...
    command: -D -V
    environment:
      JWT_SIGNER_PUBLIC_KEY: "$INDRA_NATS_JWT_SIGNER_PUBKEY"
      NODE_TLS_REJECT_UNAUTHORIZED: 0
    logging:
      driver: "json-file"
      options:
