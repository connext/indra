...
POSTGRES_HOST="${project}_database_hub_watcher"
DATABASE="$POSTGRES_HOST:5432"
DATABASE_URL="postgresql://$POSTGRES_USER:$POSTGRES_PASSWORD@$DATABASE/$POSTGRES_DB"

REDIS_HOST="${project}_redis_hub_watcher"
REDIS="$REDIS_HOST:6379"
REDIS_URL="redis://$REDIS"

ETHPROVIDER_HOST="${project}_ethprovider_hub_watcher"
...
function cleanup {
  ...
  docker container stop ${project}_hub_watcher 2> /dev/null || true
  echo;echo "Goodbye!"
}
trap cleanup EXIT
...
echo "Activating hub watcher.."


docker run --tty --name ${project}_hub_watcher --network=$project \
