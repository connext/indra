...
echo "Requests will be forwarded to: $indra_url"
...
echo;echo "Running start script: bash start-daicard.sh $daicard $indra_url"
ssh -i $ssh_key $user@$daicard "bash -c '
  docker service rm daicard_proxy
  bash start-daicard.sh $daicard $indra_url
'"
