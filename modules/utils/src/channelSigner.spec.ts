...
describe("ChannelSigner", () => {
  ...
  it("should sign Channel messages", async () => {
    ...
    const sig = await signer.signMessage(channelMessage);
    expect(getEthSignatureError(sig)).toBeUndefined();
    expect(recoverAddressFromChannelMessage(channelMessage, sig)).toBeTruthy();
  });
  ...
  it("should be able to decrypt stuff it encrypts", async () => {
    ...
    const encrypted = await sender.encrypt(encryptMessageSM, recipient.publicKey);
    const decrypted = await recipient.decrypt(encrypted);
    expect(encryptMessageSM).toEqual(decrypted);
  });

  it("should decrypt messages longer than 15 chars", async () => {
    const sender = getRandomChannelSigner();
    const recipient = getRandomChannelSigner();
    const encrypted = await sender.encrypt(encryptMessageLG, recipient.publicKey);
    const decrypted = await recipient.decrypt(encrypted);
    expect(encryptMessageLG).toEqual(decrypted);
  });
  ...
  it("should have encrypt/decrypt that are compatible with eth-crypto", async () => {
    ...
    const myEncrypted = await mySigner.encrypt(encryptMessageSM, ethSignerPublicKey);
    const ethEncrypted = EthCrypto.cipher.stringify(
      await EthCrypto.encryptWithPublicKey(mySigner.publicKey.replace(/^0x/, ""), encryptMessageSM),
    );
    ...
    expect(myDecrypted).toEqual(encryptMessageSM);
  });
