...
export class ConnextListener extends ConnextEventEmitter {
  ...
  private registerAvailabilitySubscription = async (): Promise<void> => {
    ...
    await this.connext.channelProvider.node.messaging.subscribe(
      subject,
      async (msg: any): Promise<any> => {
        ...
        const response = true;
        this.connext.channelProvider.node.messaging.publish(msg.reply, {
          err: null,
          response,
        });
      },
  ...
  private registerLinkedTransferSubscription = async (): Promise<void> => {
    ...
    await this.connext.channelProvider.node.messaging.subscribe(subject, async (msg: any) => {
  ...
  private handleAppProposal = async (
    ...
    try {
      ...
      commonAppProposalValidation(
        ...
        this.connext.channelProvider.node.config.supportedTokenAddresses,
      );
