 ...
 */
const assertSigPresence = (update: UpdateRequest, x?: 'sigHub' | 'sigUser'): any => {
  if (x && !update[x]) {
...
export class StateUpdateController extends AbstractController {
  ...
  public async start(): Promise<any> {
    this.unsubscribe = await this.watchStore(
      ...
      (item: any): any => this.syncOneItemFromStore(item),
    )
  }
   ...
   */
  private queueAction(action: Action<any>): void {
  ...
  private async syncOneItemFromStore(item: SyncResult | undefined): Promise<void> {
    if (!item) return
  ...
  public async handleSyncItem(item: SyncResult): Promise<any> {
    ...
    if (item.type === 'thread') { // handle thread updates
      ...
      if (!item.update.id) {
        throw new Error(
          `Uh oh! Thread update should definitely have the id at this point! ` +
          `Sync item: ${JSON.stringify(item)}`)
      }
      ...
      const prevThreadState = persistent.activeThreads.filter(
        (t: any): boolean => t.threadId === newThreadState.threadId
          && t.sender === newThreadState.sender
          && t.receiver === newThreadState.receiver)

      if (prevThreadState.length !== 1) {
        throw new Error(
          `Error finding previous thread state corresponding to given new thread state. ` +
          `New state: ${JSON.stringify(newThreadState)}. ${
            prevThreadState.length === 0
              ? 'No thread found.'
              : `Multiple threads found ${JSON.stringify(prevThreadState)}`
          }`)
      }
    ...
    if (
      update.reason !== 'Invalidation'
      && update.txCount
      && update.txCount !== prevState.txCountGlobal + 1
    ) {
      throw new Error(
    ...
    if (update.sigHub && update.sigUser) {
      ...
      if (update.reason.includes('Thread')) {
        ...
        const threadHistory = connextState.persistent.threadHistory.map((t: any): any => {
        ...
        const activeInitialThreadStates = update.reason === 'OpenThread'
          ...
          : prevActiveInitialThreads.filter(
            (t: any): boolean => t.sender !== args.sender
                && t.receiver !== args.receiver
                && t.threadId !== args.threadId)

        const activeThreads = update.reason === 'OpenThread'
          ? prevActiveThreads.concat([args])
          : prevActiveThreads.filter(
            (t: any): boolean => t.sender !== args.sender
              && t.receiver !== args.receiver
              && t.threadId !== args.threadId)
  ...
  public updateHandlers: StateUpdateHandlers = {
    ...
    'CloseThread': async (prev: any, update: any): Promise<any> => {
      ...
      if (update.args.sender !== prev.user) {
        throw new Error(
          `Something funny is going on. Client has received a close thread update from the hub, ` +
          `where the client is the receiver. Somehow, this was missing a user sig. ` +
      ...
      const args = update.args as ThreadState
      const initialState = persistent.activeInitialThreadStates.filter(
        (t: any): boolean => t.sender === args.sender
          && t.receiver === args.receiver
          && t.threadId === args.threadId)[0]
      const activeThread = persistent.activeThreads.filter(
        (t: any): boolean => t.sender === args.sender
          && t.receiver === args.receiver
          && t.threadId === args.threadId)[0]
      if (!activeThread || !initialState) {
        throw new Error(
          `Hub is trying to close thread we have no local record of. ` +
