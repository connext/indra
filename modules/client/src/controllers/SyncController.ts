...
export default class SyncController extends AbstractController {
  ...
  private async checkCurrentStateTimeoutAndInvalidate() {
    ...
    const block = await this.connext.opts.web3.eth.getBlock('latest')
    ...
    const msg = (
      ...
      `DidUpdateChannel events have been seen since block ${block.number - 1000}`
    )
  ...
  public async sendInvalidation(
    ...
    if (updateToInvalidate.txCount !== channel.txCountGlobal && updateToInvalidate.reason.startsWith("ProposePending")) {
      throw new Error(
        `Cannot invalidate 'ProposePending*' type updates that have been built on top of.`
      )
    }

    if (updateToInvalidate.sigUser) {
      throw new Error(
        `Refusing to invalidate an update we have already signed: ` +
