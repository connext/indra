...
describe('SyncController', () => {
  describe('mergeSyncResults', () => {
    ...
    const chanTests = [
      ...
      ['mixed only channel', [
        mkChanResult(1), mkChanResult(2)], [mkChanResult(3), mkChanResult(2)],
      ],
    ]
      ...
      it(name as any, () => {
        ...
        for (let i = 0; i < actual.length; i += 1) {
          assert.containSubset(
            actual[i].update,
            { createdOn: (expected[i] as any).update.createdOn },
  ...
  describe.skip('findBlockNearestTimeout', () => {
  ...
  describe.skip('ConfirmPending unit tests', () => {
    const user = mkAddress('0xUUU')
    let connext: MockConnextInternal
    const mockStore = new MockStore()
