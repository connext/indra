...
export function objValuesStringToBigNum(x: any, fields: string[]): any {
  ...
  Object.entries(x).forEach(([key, val]) => {
    if (!val) {
      ...
      (res as any)[key] = fields.includes(key) ? new BigNumber(val as any): val
    }
  })
  return res
}

export function channelRowStringToBigNum(r: ChannelRow): ChannelRowBigNumber {
  return {
    ...r,
...
export function channelRowBigNumToString(r: ChannelRowBigNumber): ChannelRow {
  return {
    ...r,
...
export function channelStateUpdateRowBigNumToString(
  update: ChannelStateUpdateRowBigNumber,
...
export function channelStateUpdateRowStringToBigNum(
...
): ChannelStateUpdateRowBigNumber {
  let { args } = update
  let argsBigNum
  switch (update.reason) {
    ...
    case 'ProposePendingWithdrawal':
      ...
      break
    default: // TODO: implement other 5 cases too
      argsBigNum = objValuesStringToBigNum(args, [])
  }
  return {
    ...update,
