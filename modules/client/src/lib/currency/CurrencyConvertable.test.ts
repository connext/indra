...
import { default as generateExchangeRates } from '../generateExchangeRates'
...
describe('CurrencyConvertable', () => {
  ...
  const initialState = new ConnextState()
  initialState.runtime.exchangeRate = {
    lastUpdated: new Date(),
    rates: generateExchangeRates('420'),
  }

  const store = redux.createStore(reducers, initialState)

  it('should convert to a new currency with the CurrencyConvertable.to method', () => {
    const eth = new CurrencyConvertable(CurrencyType.ETH, '100', () => getExchangeRates(store))
  ...
  describe ('currency convertable should not lose precision when converting numbers with under 64 significant digits', () => {
    ...
    function testIt(tc: TestCase) {
      const eth = new CurrencyConvertable(CurrencyType.ETH, tc, () => getExchangeRates(store))
