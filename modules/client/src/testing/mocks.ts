...
import { EMPTY_ROOT_HASH } from "@src/lib/constants";
...
export class MockConnextInternal extends ConnextInternal {
  ...
  constructor(opts: Partial<ConnextClientOptions> = {}) {
    ...
    // TODO: fix mocks better
    // @ts-ignore NOTE: ignoring lack of mock type overlap,
...
export class MockChannelManager implements IChannelManager {
  ...
  getChannelDetails(user: string) {
    return {
      ...
      threadCount: 0,
      exitInitiator: user,
      channelClosingTime: Math.floor(Date.now() / 1000),
      status: "CS_OPEN"
    } as any;
  }
  ...
  assertCalled(method: keyof MockChannelManager, ...args: any[]) {
    ...
    assert.fail(
      `No contract methods calls matching '${method}(${JSON.stringify(
        args
      )})' were made!\n` +
        `Method calls:\n${this.contractMethodCalls
...
export class MockHub implements IHubAPIClient {
      ...
      return {
        ...
        update: {
          ...
          txCount: (p.update as UpdateRequest).sigUser
            ? (p.update as UpdateRequest).txCount!
            : (p.update as UpdateRequest).txCount! + 1
        } as UpdateRequest
      } as SyncResult;
    });
  ...
  async updateHub(
    ...
    lastThreadUpdateId: number
  ): Promise<{ error: null; updates: SyncResult[] }> {
    this.receivedUpdateRequests = [...this.receivedUpdateRequests, ...updates];
    return {
      error: null,
      updates: []
    };
  ...
  assertReceivedUpdate(expected: PartialUpdateRequest) {
    ...
    assert.fail(
      `Hub did not recieve any updates matching ${JSON.stringify(
        expected
      )}. Got:\n` +
...
export class MockStore {
    ...
    this._initialState = {
      ...
      persistent: {
        ...
        channel: getChannelState(
          "empty",
          { txCountChain: 0, txCountGlobal: 0 },
          overrides
        )
      }
    };
  ...
  public addThread = (overrides: PartialSignedOrSuccinctThread) => {
    ...
    const initialThread: ThreadState = convertThreadState("str", {
      ...
      balanceTokenSender: threadBN.balanceTokenSender.add(
        threadBN.balanceTokenReceiver
      ),
      balanceWeiSender: threadBN.balanceWeiSender.add(
