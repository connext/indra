...
export class LockService {
  ...
  async lockedOperation(
    ...
    this.log.error(`Acquiring lock for ${lockName} ${Date.now()}`);
    return new Promise((resolve: any, reject: any): any => {
      this.redlockClient
        .lock(lockName, hardcodedTTL)
        .then(async (lock: Redlock.Lock) => {
          const acquiredAt = Date.now();
          this.log.error(`Acquired lock at ${acquiredAt} for ${lockName}:`);
          let retVal: any;
          try {
            // run callback
            ...
            // unlock
            this.log.error(`Releasing lock for ${lock.resource} with secret ${lock.value}`);
            lock
              .unlock()
              .then(() => resolve(retVal))
              .catch((e: any) => {
  ...
  async acquireLock(lockName: string, lockTTL: number = LOCK_SERVICE_TTL): Promise<string> {
    const hardcodedTTL = LOCK_SERVICE_TTL;
    this.log.debug(`Using lock ttl of ${hardcodedTTL / 1000} seconds`);
    this.log.error(`Acquiring lock for ${lockName} at ${Date.now()}`);
    return new Promise((resolve: any, reject: any): any => {
      this.redlockClient
        .lock(lockName, hardcodedTTL)
        .then((lock: Lock) => {
          this.log.error(`Acquired lock for ${lock.resource} with secret ${lock.value}`);
          resolve(lock.value);
        })
        .catch((e: any) => {
  ...
  async releaseLock(lockName: string, lockValue: string): Promise<void> {
    ...
    return new Promise((resolve: any, reject: any): any => {
      this.redlockClient
        ...
        .then(() => {
          this.log.error(`Released lock for ${lockName}`);
          resolve();
        })
        .catch((e: any) => {
