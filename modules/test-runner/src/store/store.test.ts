import { ConnextStore, MemoryStorage, PATH_CHANNEL, StorePair } from "@connext/store";
...
import { MEMORYSTORAGE } from "@connext/types";

describe("ConnextStore", () => {
  let store: ConnextStore;

  beforeEach(async () => {
    const testStore = createConnextStore(MEMORYSTORAGE);
    store = testStore;
  });
  ...
  //   ];
  //   await store.saveStateChannel(pairs);
  //   const expected = {
