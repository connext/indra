import { EventNames, MethodParams, WithdrawStartedMessage } from "@connext/types";
...
import { WithdrawConfirmationMessage } from "../../types";

import { toBeEq, toBeLt } from "../bignumber-jest-matcher";
...
function confirmWithdrawalMessages(
  ...
  // initiator messages
  initiator.once(EventNames.WITHDRAWAL_CONFIRMED_EVENT, (msg: WithdrawConfirmationMessage) => {
    assertNodeMessage(
      msg,
      {
  ...
  initiator.once(EventNames.WITHDRAWAL_STARTED_EVENT, (msg: any) => {
    assertNodeMessage(msg, startedMsg, ["data.txHash"]);
  });

  responder.once(EventNames.WITHDRAWAL_STARTED_EVENT, (msg: WithdrawStartedMessage) => {
...
describe("Node method follows spec - withdraw", () => {
  ...
  it("has the right balance for both parties after withdrawal", async () => {
    ...
    confirmWithdrawalMessages(nodeA, nodeB, withdrawReq.parameters as MethodParams.Withdraw);
  ...
  it("Node A produces a withdraw commitment and non-Node A submits the commitment to the network", async () => {
    ...
    confirmWithdrawalMessages(
      nodeA,
      nodeB,
      withdrawCommitmentReq.parameters as MethodParams.Withdraw,
    );
