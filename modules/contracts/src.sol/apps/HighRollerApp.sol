...
contract HighRollerApp is CounterfactualApp {
  enum ActionType {COMMIT_TO_HASH, COMMIT_TO_NUM, REVEAL_NUM}
    ...
    returns (bytes memory)
  {
    ...
    if (action.actionType == ActionType.COMMIT_TO_HASH) {
      ...
      require(action.number != 0, "It is considered invalid to use 0 as the number.");
      ...
      bytes32 expectedCommitHash = keccak256(abi.encodePacked(action.actionHash, action.number));
    ...
    returns (bytes memory)
  {
    ...
    // Co-operative case
    return abi.encode(getWinningAmounts(appState.playerFirstNumber, appState.playerSecondNumber));
  }
  ...
  function getPlayerRolls(
    bytes32 randomness // NOTE: This is used in app-root.tsx for the clientside dapp
  )
    public
    pure
    returns (
  ...
  function calculateRandomSalt(uint256 num1, uint256 num2) internal pure returns (bytes32) {
  ...
  function bytes8toDiceRoll(bytes8 q) internal pure returns (uint8) {
