...
contract TicTacToeApp is CounterfactualApp {
  enum ActionType {PLAY, PLAY_AND_WIN, PLAY_AND_DRAW, DRAW}

  enum WinClaimType {COL, ROW, DIAG, CROSS_DIAG}
  ...
  function playMove(
    ...
    uint256 x,
    uint256 y
  ) internal pure returns (AppState memory) {
  ...
  function assertBoardIsFull(AppState memory preState) internal pure {
