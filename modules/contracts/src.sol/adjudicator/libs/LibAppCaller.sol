...
contract LibAppCaller {
  ...
  function isStateTerminal(address appDefinition, bytes memory appState)
    internal
    view
    returns (bool)
  {
  ...
  function getTurnTaker(
    ...
    return CounterfactualApp(appDefinition).getTurnTaker(appState, participants);
  }
  ...
  /// @param appDefinition An address of an app definition to call
  /// @param appState The ABI encoded version of some application state
  function computeOutcome(address appDefinition, bytes memory appState)
    internal
    view
