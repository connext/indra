...
contract LibStateChannelApp is LibDispute {
  ...
  function isDisputable(AppChallenge memory appChallenge) public view returns (bool) {
    return
      appChallenge.status == ChallengeStatus.NO_CHALLENGE ||
      (appChallenge.status == ChallengeStatus.IN_DISPUTE && !hasPassed(appChallenge.finalizesAt));
  }

  /// @dev Checks an outcome for a challenge has been set
  /// @param appChallenge the app challenge to check
  function isOutcomeSet(AppChallenge memory appChallenge) public pure returns (bool) {
  ...
  function isProgressable(AppChallenge memory appChallenge, uint256 defaultTimeout)
    public
    view
    returns (bool)
  {
  ...
  /// @param appChallenge the app challenge to check
  /// @param defaultTimeout the app instance's default timeout
  function isCancellable(AppChallenge memory appChallenge, uint256 defaultTimeout)
    public
    view
    returns (bool)
  {
  ...
  /// @param appChallenge the app challenge to check
  /// @param defaultTimeout the app instance's default timeout
  function isFinalized(AppChallenge memory appChallenge, uint256 defaultTimeout)
    public
    view
    returns (bool)
  {
  ...
  function verifySignatures(
    ...
    for (uint256 i = 0; i < signers.length; i++) {
      require(signers[i] == txHash.verifyChannelMessage(signatures[i]), "Invalid signature");
    }
    return true;
  }
}
