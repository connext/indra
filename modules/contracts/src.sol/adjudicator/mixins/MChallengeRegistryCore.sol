...
contract MChallengeRegistryCore is LibCommitment, LibStateChannelApp, LibAppCaller {
  ...
  function appStateToHash(bytes memory appState) internal pure returns (bytes32) {
  ...
  function appIdentityToHash(AppIdentity memory appIdentity) internal pure returns (bytes32) {
    return
      keccak256(
        abi.encodePacked(
  ...
  /// @param versionNumber The versionNumber corresponding to the version of the state
  /// @return A bytes32 hash of the RLP encoded arguments
  function computeCancelDisputeHash(bytes32 identityHash, uint256 versionNumber)
    internal
    pure
    returns (bytes32)
  {
    return
      keccak256(
        abi.encodePacked(uint8(CommitmentTarget.CANCEL_DISPUTE), identityHash, versionNumber)
      );
  }
