...
contract MixinChallengeRegistryCore is MChallengeRegistryCore {
  ...
  function getAppChallenge(bytes32 identityHash) external view returns (AppChallenge memory) {
  ...
  function getOutcome(bytes32 identityHash) external view returns (bytes memory) {
    AppChallenge storage challenge = appChallenges[identityHash];
    require(isOutcomeSet(challenge), "Outcome hasn't been set yet");
